{ pkgs, ... }: let
  npinsed-forgecode = (pkgs.npinsed {input=./npins/sources.json;}).forgecode;
  forgecode = (pkgs.flake-compat {src = npinsed-forgecode;}).defaultNix.default
    ...
    .overrideAttrs (_old: rec {
      ...
      version = "${npinsed-forgecode.version}-dev";
      APP_VERSION=version;
